...
if (str_contains($full_url[-1], "/")) {
	$sql = "SELECT a.animal_id, a.country_id FROM animal_countries a";
	$result = returnFromSQL($dbh, $sql);

	echo json_encode((object)[
		"status" => 200,
		"success" => true,
		"results" => returnMultiList($result, "country_id", "animal_id")
	]);
	http_response_code(200);
	exit;

} else if (preg_match("/^\d+$/", $id) && (int)$id>=0) {
	$sql = "SELECT a.animal_id, a.country_id FROM animal_countries a WHERE a.animal_id = :id";
	...
	if (isset($result[0]["animal_id"])) {
		echo json_encode((object)[
			"status" => 200,
			"success" => true,
			"result" => returnSingleList($result, "country_id")
		]);
		http_response_code(200);
		exit;
	}
...
echo json_encode((object)[
	"status" => 404,
	"success" => false,
	"message" => "No animal country found with that ID."
]);
