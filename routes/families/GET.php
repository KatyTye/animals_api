...
if (str_contains($full_url[-1], "/")) {
	$sql = "SELECT a.id, a.animal_name FROM families a";
	...
	$sql = "SELECT a.id, a.animal_name FROM families a WHERE a.id = :id";
	$result = returnBoundFromSQL($dbh, $sql, "id", $id);

	if (isset($result[0]["animal_name"])) {
		echo json_encode((object)[
			"status" => 200,
			"success" => true,
			"result" => (object)[
				"id" => $result[0]["id"],
				"animal_name" => $result[0]["animal_name"]
			]
		]);
		http_response_code(200);
		exit;
...
echo json_encode((object)[
	"status" => 404,
	"success" => false,
	"message" => "No family found with that ID."
]);
