...
$allowed = [
    "key_name", "encrypted_password","required_auth"
];
...
$sql = "UPDATE api_keys SET `$field` = :val WHERE id = :id";
...
if ($stmt->rowCount() > 0) {
    echo json_encode((object)[
        "status" => 200,
        "success" => true,
        "message" => "The key field ".$headers["field"]." has been updated successfully."
    ]);
    ...
    echo json_encode((object)[
        "status" => 204,
        "success" => false,
        "message" => "Failed to update the field ".$headers["field"]." for the key."
    ]);
